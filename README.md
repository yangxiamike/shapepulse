...
[系统定位](#验证结论与系统定位) · [设计动机](#设计动机与研究假设) · [功能预览](#功能预览) · [三分钟启动](#三分钟启动) · [页面入口](#页面入口) · [如何解读](#如何解读) · [数据边界](#数据与隐私边界)
...
**一句话定位：ShapePulse 是个股与行业的状态识别、现状监测系统，不是未来涨跌预测系统。**

这里的 A 指“刚突破”，B 指由其余三个趋势型模板共同描述的“趋势承接”。现有实验支持以下用法：

| 能力 | 正确用途 | 验证结果 |
| --- | --- | --- |
| 个股 A | 发现刚出现新突破形态的候选，供后续研究 | A 出现后 40 日内进入 B 的实际比例为 **39.31%**，共同时间移位 null 为 **5.19%**，超额 **34.11pp**；首次进入 B 的中位时间为 **14 个交易日** |
| 个股 B | 确认个股当前已进入趋势承接状态 | 用于状态确认，不等于未来还会继续上涨 |
| 行业 B | 描述当前主线是否健康、开始松动或已确认降温 | 对独立价格退潮事件的覆盖率为 **68.2%（30/44）**；命中事件中位滞后 **18 日**，确认时已完成跌幅中位数 **65.7%** |
| 龙头退出 B | 辅助观察龙头与行业状态的先后变化 | 时序特异性偏弱，只作解释线索，不作为行业退潮的因果证据或独立警报 |

行业 B 更适合确认“已经发生到什么阶段”，不适合预测“接下来是否继续走弱”：正式 B 出现后价格继续走弱的精确率只有 **14.9%**。行业 A 涌现、行业高转化主线确认、外部 A/B 轮动和 A 修复均未通过升级门槛，当前不应据此判断资金迁移、下一主线或未来涨跌。

日常使用时，可以先用个股 A 缩小新突破候选范围，再用个股 B 确认趋势状态；随后查看行业 B，区分主线仍健康、开始松动还是已经确认降温。龙头退出 B 只作为旁证，最终判断仍需结合真实 K 线、基本面、流动性与风险信息。

详细研究证据：

- [两状态—行业兴衰扩样严谨验证](docs/research/validation-evidence-20260731.md#两状态行业兴衰扩样严谨验证)
- [主线走弱与相对替代验证](docs/research/validation-evidence-20260731.md#主线走弱与相对替代验证)
- [主线退潮时序验证](docs/research/validation-evidence-20260731.md#主线退潮时序验证)

> [!NOTE]
> 上述结果是特定样本、冻结规则和本地数据口径下的条件关联，不是因果结论，也没有验证交易收益。行业价格由历史成分股等权合成，仅用于研究阶段确认，**不是正式申万行业指数**。
...
进一步地，ShapePulse 将每个模板的 Top100 按行业聚合，用价格形态的行业宽度描述市场状态及其时间变化。实证结果支持把“刚突破”用于发现个股候选、把其余三个趋势型模板用于确认个股趋势状态；行业层则应以 B 描述当前主线健康、松动和已确认降温。行业 A 涌现、A/B 轮动等更强解释尚未通过验证，具体边界见上方“验证结论与系统定位”。
